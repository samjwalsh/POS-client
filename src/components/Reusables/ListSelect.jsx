...
const useListSelect = () => {
  ...
  const optionsHTML = options.map((option) => {
    return (
      <div
        className='p-4 cnter-items primary  text-xl w-full'
    ...
    if (promise === null) return;
      ...
      return (
        <div className='fixed h-screen w-screen z-50'>
          ...
          <div className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 background border border-colour rnd  flex flex-col gap-2 p-2 rndmd min-w-[15rem]'>
            {optionsHTML}
          </div>
        </div>
      );
