  ...
  function createKeypadHTML() {
    return (
      <div
        ...
        onClick={(event) => handleKeypadClick(event)}onTouchMove={(event) => handleKeypadClick(event)}>
  ...
  const keypadHTML = () => {
    ...
    return (
      <div className='fixed h-screen w-screen z-20'>
        ...
        <div className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 backgroundcolour border border-colour w-4/12 h-5/6 rounded-md shadow'>
          {createKeypadHTML()}
        </div>
      </div>
    );
